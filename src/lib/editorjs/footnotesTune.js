...
function detachNotesFromStaticPool(BaseFootnotesTune, wrapper) {
  ...
  const blockNotes = BaseFootnotesTune.notes.filter((note) =>
    wrapper.contains(note.node),
  );
  BaseFootnotesTune.notes = BaseFootnotesTune.notes.filter(
    (note) => !wrapper.contains(note.node),
  );
...
export async function loadFootnotesTune() {
  ...
  if (typeof BaseFootnotesTune !== "function") {
    throw new Error("FootnotesTune failed to load.");
  }
  ...
  class FootnotesTune extends BaseFootnotesTune {
    ...
    save() {
      const supElements = getFootnoteElements(this.wrapper);

      return this.notes
        .filter((note) => supElements.includes(note.node))
        .map((note) => note.save());
    }

    insertNote(newNote) {
      const noteByNode = new Map(this.notes.map((note) => [note.node, note]));
      noteByNode.set(newNote.node, newNote);

      this.notes = getFootnoteElements(this.wrapper)
        .map((sup) => noteByNode.get(sup))
        .filter(Boolean);
      this.updateIndices();
    }
    ...
    contentDidMutated(mutationsList) {
      const footnotesChanged = mutationsList.some((record) => {
    ...
    syncNotesFromDom() {
      const noteByNode = new Map(
        this.notes.filter((note) => this.wrapper.contains(note.node)).map((note) => [note.node, note]),
      );
